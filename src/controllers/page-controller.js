...
const renderCards = (container, cards, popupContainer, onDataChange) => {
  return cards.map((film) => {
    const movieController = new MovieController(container, popupContainer, onDataChange);
...
export default class PageController {
  ...
  _renderShowMoreButton() {
    ...
    const onShowMoreButtonClick = () => {
      ...
      const newCards = renderCards(this._filmsListContainer, sortedFilms.slice(previousCardsCount, this._showingCardsCount),
          this._popupContainer, this._onDataChange);
  ...
  _onSortTypeChange(sortType) {
    ...
    this._showingCardsCount = SHOWING_CARDS_AMOUNT_ON_START;
    const newCards = renderCards(this._filmsListContainer, sortedFilms.slice(0, this._showingCardsCount), this._popupContainer, this._onDataChange);
  ...
  _renderExtraFilms() {
    ...
    filmListExtraElements.forEach((listElement) => {
      ...
      const newCards = renderCards(extraFilmsContainer, extraFilms[count], this._popupContainer, this._onDataChange);
  ...
  render(films) {
    ...
    const newCards = renderCards(this._filmsListContainer, films.slice(0, this._showingCardsCount), this._popupContainer, this._onDataChange);
    this._showedMovieControllers = this._showedMovieControllers.concat(newCards);
